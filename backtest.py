...
def align_1h_to_15m(df_1h, df_15m):
    df_1h_small = df_1h[["Open_Time", "Close", "ema200", "rsi", "macd_hist"]].copy()
...
def main():
    print("=" * 70)
    print("KURMAY BACKTEST v3 (SNIPER - HACİM/MACD/ATR KORUMALI) BAŞLIYOR...")
    print("=" * 70)
    ...
    rename_map = {"Close_15m": "Close", "rsi_15m": "rsi", "Close_1h": "Close_1h", "rsi_1h": "rsi_1h"}
    ...
    equity_curve, trades = [], []
    max_equity, max_drawdown = initial_equity, 0.0

    for i in range(850, len(df)):
        row = df.iloc[i]
        row_15m = {
            "Open_Time": row["Open_Time"], "Close": float(row["Close"]),
            "ema21": float(row["ema21"]), "rsi": float(row["rsi"]),
            "bb_upper": float(row["bb_upper"]), "bb_lower": float(row["bb_lower"]),
            "atr": float(row["atr"]), "obv": float(row["obv"]), "obv_ema": float(row["obv_ema"]),
        }
        row_1h = {
            "Close_1h": float(row["Close_1h"]), "ema200": float(row["ema200"]),
            "rsi_1h": float(row["rsi_1h"]), "macd_hist": float(row["macd_hist"])
        }

        signal = generate_signal(row_15m, row_1h, trade_try, trade_btc)
        base_price = row_15m["Close"]

        if signal["action"] == "BUY" and signal["amount_try"] > 0 and trade_try >= signal["amount_try"]:
            ...
            trades.append({"action": "BUY", "price": exec_price, "amount_try": amount_try, "btc_change": btc_bought})

        elif signal["action"] == "SELL" and signal["amount_btc"] > 0 and trade_btc >= signal["amount_btc"]:
            exec_price = base_price * (1 - SLIPPAGE)
            ...
            trade_try += net_try
            trades.append({"action": "SELL", "price": exec_price, "amount_try": net_try, "btc_change": -amount_btc})
    ...
    print("\n" + "=" * 70)
    print("BACKTEST SONUCU (V3 SNIPER)")
    print("=" * 70)
